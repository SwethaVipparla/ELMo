...
def run_sweep(cfg=None):
    with mywandb.init(config=cfg):
        ...
        mywandb.log({'batch_size': BATCH_SIZE})
        ...
        mywandb.watch(elmo, log_freq=100)
...
sweep_id = mywandb.sweep(cfg, project='ELMO')
mywandb.agent(sweep_id, run_sweep, count=20)
